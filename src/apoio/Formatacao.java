...
public class Formatacao {
    ...
    // método para formatação do CEP inserido
    ...
    // formatação para CPF
    public static void formatarCpf(JFormattedTextField campo) { // passa como parametro o campo que será formatado no método
        try {
            MaskFormatter m = new MaskFormatter(); // criar um objeto de formatação com mascaras
            m.setPlaceholderCharacter(' ');
            m.setMask("###.###.###-##"); // # significa que só números podem ser inseridos. . e - são as contantes e estão presentes no nosso JFormattedTextField
            campo.setFormatterFactory(null); // três linhas finais são obrigatórias, pois realizam configs importantes no objeto campo que é o patrametro do método
            campo.setFormatterFactory(new DefaultFormatterFactory(m));
            campo.setValue(null);
        } catch (Exception e) {
            System.err.println(e);
